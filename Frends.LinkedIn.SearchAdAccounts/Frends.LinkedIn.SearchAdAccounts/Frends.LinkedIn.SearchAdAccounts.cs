...
public static class LinkedIn
{
    ...
    public static async Task<Result> SearchAdAccounts([PropertyTab] Filter filter, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
    ...
    {
        var sb = new StringBuilder($"https://api.linkedin.com/rest/adAccounts");
        ...
        var typeFilters = options.TypeFilters != null ? options.TypeFilters.Select(x => x.AccountType.ToString()).ToList() : new List<string>();
        if (typeFilters.Any())
            dict["type"] = typeFilters;

        var statusFilters = options.StatusFilters != null ? options.StatusFilters.Select(x => x.AccountStatus.ToString()).ToList() : new List<string>();
